-- Fix 1: Add RLS policies to style_leaderboard view
-- Since this is a view showing aggregated public data, we'll allow public read access
-- but restrict who can modify the underlying data

-- Enable RLS on style_leaderboard (if it's a table)
ALTER TABLE public.style_leaderboard ENABLE ROW LEVEL SECURITY;

-- Allow everyone to view the leaderboard (public data)
CREATE POLICY "Anyone can view style leaderboard" 
ON public.style_leaderboard 
FOR SELECT 
USING (true);
